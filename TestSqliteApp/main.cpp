...
repo::dal::Product selectProduct(repo::ISession& s)
{
	std::stringstream selectProduct;
	selectProduct << "select * from " << repo::dal::Product::TableName << " limit 1;";
	repo::dal::Product result;
...
void copyData(repo::ISession& s)
{
	...
	selectRegion3 << "select * from " << repo::dal::Region::TableName << " where " << repo::dal::Region::Field::name << " = 'region 3' limit 1";
	s.ExecSql(selectRegion3.str().c_str(), [&s](const repo::IRow& row) {
		repo::TransactionPatch tr;
		repo::dal::Region r(row);
		repo::dal::Region copy = r.Copy();
...
void editData(repo::ISession& s)
{
	...
	selectProduct1 << "select * from " << repo::dal::Product::TableName << " where " << repo::dal::Product::Field::name << " = 'product 1' limit 1;";
	s.ExecSql(selectProduct1.str().c_str(), [&s](const repo::IRow& row) {
		repo::TransactionPatch tr;
		repo::dal::Product p(row);
...
void selectData(repo::ISession& s)
{
	...
	selectInspections << "select * from " << repo::dal::Inspection::TableName;
	s.ExecSql(selectInspections.str().c_str(), [](const repo::IRow& row) {
		repo::dal::Inspection i(row);
		std::cout << "------------------------" << std::endl;
	...
	selectRegions << "select * from " << repo::dal::Region::TableName;
