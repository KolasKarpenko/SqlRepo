...
void ZeroSchemaPatch::Apply(ISession& session) const {
	...
	createJournal << " (";
	createJournal << " id integer primary key not null auto_increment,";
	...
	createChildToParentNames << " child varchar(512) not null unique,";
...
REGISTER_PATCH(repo::mysql::ZeroSchemaPatch)
